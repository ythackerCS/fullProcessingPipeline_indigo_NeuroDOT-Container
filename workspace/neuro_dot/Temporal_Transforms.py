...
class tx4m:
    ...
    def resample_tts(data_in, info_in, omega_resample = 1, tol = 0.001, framerate = 0): # returns data_out and info_out
        ...
        fract = sym.nsimplify((omega_resample/framerate), tolerance = tol, rational = True) #in matlab tolerance is set to 1e-5, in order to reproduce outputs from neuroDOT matlab, tolerance is set to 0.001 in neuroDOT
        N = fract.numerator
        D = fract.denominator
